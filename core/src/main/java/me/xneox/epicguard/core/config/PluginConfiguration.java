...
public class PluginConfiguration {
  ...
  @Comment("Alert online staff members when a player is blocked by a check.")
  private StaffAlerts staffAlerts = new StaffAlerts();

  @Comment("Execute console commands when a player is blocked by a check.")
  private CommandsOnDetection commandsOnDetection = new CommandsOnDetection();
  ...
  public static class AccountLimitCheck {
    ...
    @Comment("Limit of accounts connected per one IP address without any attack happening.\n"
        + "A list of accounts per IP is tracked and stored in the database.")
    private int limit = 3;

    @Comment("Limit of accounts connected per one IP address during a bot-attack.\n"
        + "A list of accounts per IP is tracked and stored in the database.")
  ...
  public static class StaffAlerts {
    @Comment("Enable staff alert notifications when a player is blocked.")
    private boolean enabled = true;
  ...
  @ConfigSerializable
  public static class CommandsOnDetection {
    @Comment("Enable executing console commands when a player is blocked.")
    private boolean enabled = false;

    @Comment("Commands to execute when a player is blocked.\n"
        + "Placeholders: {PLAYER} - player name, {ADDRESS} - IP address, {CHECK} - check name")
    private List<String> commands = Collections.singletonList(
        "kick {PLAYER} You have been blocked by EpicGuardPlus ({CHECK})");
  ...
  public static class Misc {
    ...
    @Comment("IP addresses listed here will bypass all checks.\n"
        + "Useful for known safe IPs, proxies, or bot panels.")
    private List<String> bypassAddresses = new ArrayList<>();

    @Comment("MaxMind GeoLite2 license key for downloading GeoIP databases.\n"
        + "Get your free license key at: https://www.maxmind.com/en/geolite2/signup\n"
        + "After signing up, go to Account > Manage License Keys > Generate New License Key.")
    ...
    public List<String> bypassAddresses() {
      return this.bypassAddresses;
    }
  ...
  public StaffAlerts staffAlerts() {
    return this.staffAlerts;
  }

  public CommandsOnDetection commandsOnDetection() {
    return this.commandsOnDetection;
  }
