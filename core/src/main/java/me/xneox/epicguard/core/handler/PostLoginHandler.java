...
public abstract class PostLoginHandler {
  ...
  public void onPostLogin(@NotNull UUID uuid, @NotNull String address) {
    // If the player has the bypass permission, auto-whitelist their address.
    if (this.epicGuard.platform().hasPermission(uuid, "epicguard.bypass")) {
      var meta = this.epicGuard.storageManager().addressMeta(address);
      if (!meta.whitelisted()) {
        meta.whitelisted(true);
        this.epicGuard.logger().info("Auto-whitelisted address " + address + " (player has epicguard.bypass)");
      }
    }
